...
myKeys =
    ...
    , ((0, xF86XK_MonBrightnessDown), spawn "sudo screen-brightness.sh -2%")
    , ((0, xF86XK_MonBrightnessUp), spawn "sudo screen-brightness.sh +2%")
    , ((mod4Mask, xK_c), spawn "~/.scripts/./context-runner.sh")
    , ((mod4Mask+shiftMask, xK_t), spawn (term ++ " --class='term-cal' --title 'Term Cal' -o window_padding_width=50 -o font_size=15 -o cursor_shape=underline -o cursor_stop_blinking_after=1 -o cursor_underline_thickness=0 sh -c '/usr/bin/cal -3w && read && exit'"))
    ...
    , ((mod4Mask+shiftMask, xK_a), spawn "pactl set-default-sink alsa_output.pci-0000_07_00.1.hdmi-stereo-extra1")
    , ((mod4Mask, xK_a), spawn "pactl set-default-sink alsa_output.pci-0000_07_00.6.analog-stereo && pactl set-default-source alsa_input.pci-0000_07_00.6.analog-stereo && pactl set-source-volume alsa_input.pci-0000_07_00.6.analog-stereo 45%")
...
myManageHook = composeAll
    [
        className =? "term-cal" --> doRectFloat (W.RationalRect 0.25 0.33 0.50 0.34)
...
myLayouts =
    layoutColumns ||| layoutCenter ||| layoutFull where
            layoutColumns = smartBorders $ avoidStruts $ myGaps $ multiCol [1] 2 0.01 (-0.5)
            layoutCenter = centerMaster (layoutColumns)
